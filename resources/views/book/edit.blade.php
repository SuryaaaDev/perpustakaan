...
<body class="bg-gray-100">
    ...
    <div class="flex py-10 justify-center items-center">
        <div class="max-w-xl p-16 rounded-xl shadow-xl bg-white">
            ...
            <form action="{{ route('book.update', $book->id) }}" method="post" class="flex flex-col gap-5" enctype="multipart/form-data">
                @csrf
                ...
                <label for="image"
                    ...
                    <input type="file" id="image"
                        ...
                        placeholder="Gambar" name="image" value="{{ $book->image }}" />
                </label>
                <label for="title"
                ...
                <div>
                    ...
                    @foreach ($types as $type)
                        <input type="checkbox" name="types[]" value="{{ $type->id }}" class="mx-2"
                            @if (in_array($type->id, old('types', $book->types->pluck('id')->toArray()))) checked {{-- menampilkan check yang sudah di check --}} @endif>{{ $type->name }}</input><br>
                    @endforeach
                </div>
                <button
